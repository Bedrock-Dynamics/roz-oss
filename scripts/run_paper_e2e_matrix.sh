...
for arg in "$@"; do
  case "$arg" in
    ...
    --with-manipulator)
      ;;
    --skip-manipulator)
      export ROZ_SKIP_MANIPULATOR_PAPER_TEST=1
      ;;
    --help)
  cat <<'EOF'
...
Usage:
  ...
  scripts/run_paper_e2e_matrix.sh --paper-only
  scripts/run_paper_e2e_matrix.sh --paper-only --skip-manipulator
...
run_paper() {
  ...
  if [[ "${ROZ_SKIP_MANIPULATOR_PAPER_TEST:-0}" == "1" ]]; then
    printf '\n==> skipping manipulator paper test because ROZ_SKIP_MANIPULATOR_PAPER_TEST=1\n'
  else
