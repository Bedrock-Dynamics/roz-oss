...
//! the deterministic fake manipulator backend (Plan 08).
//!
//! Coverage:
//!   1. `fake_manipulator_tick_loop_advances_positions` — boots Copper with the
...
//!   2. `fake_manipulator_estop_smoke_via_halt` — boots Copper, sends `Halt` via
...
//!   3. `hardware_*` deferred bench-rig slots (gated `#[ignore]` +
//!      `ROZ_MANIPULATOR_HARDWARE=1`):
...
/// Boot Copper against the fake manipulator IO pair and prove the sensor half
...
fn fake_manipulator_tick_loop_advances_positions() {
    ...
    // Use the fake manipulator pair directly at the IO layer to validate the
    ...
    let (actuator, mut sensor) = roz_copper::fake_manipulator::fake_manipulator_pair(&rt);

    // Drive a non-zero command into the actuator half.
    actuator
        .send(&CommandFrame { values: vec![1.0, 0.5] })
...
/// fixture.
#[tokio::test]
async fn fake_manipulator_estop_smoke_via_halt() {
    ...
    let handle =
        CopperHandle::spawn_with_policy_and_io(1.5, Arc::new(actuator), Some(Box::new(sensor)), policy, bp, None);
...
#[test]
#[ignore = "hardware bench — set ROZ_MANIPULATOR_HARDWARE=1 to run against real manipulator hardware"]
fn hardware_tiny_bounded_motion() {
    if std::env::var("ROZ_MANIPULATOR_HARDWARE").as_deref() != Ok("1") {
        eprintln!("ROZ_MANIPULATOR_HARDWARE not set — skipping hardware tiny-bounded-motion test");
        return;
    }
    unimplemented!(
        "Hardware tiny-bounded-motion deferred — owned by a future bench-rig phase. \
         Simulator-backed vertical coverage is the default robotics validation path."
    )
}
...
fn hardware_encoder_current_feedback() {
    if std::env::var("ROZ_MANIPULATOR_HARDWARE").as_deref() != Ok("1") {
        eprintln!("ROZ_MANIPULATOR_HARDWARE not set — skipping hardware encoder/current feedback test");
        return;
    }
    unimplemented!(
        "Hardware encoder/current feedback deferred — owned by a future bench-rig phase. \
...
fn hardware_channel_order_sign_units() {
    if std::env::var("ROZ_MANIPULATOR_HARDWARE").as_deref() != Ok("1") {
        eprintln!("ROZ_MANIPULATOR_HARDWARE not set — skipping hardware channel order/sign/units test");
        return;
    }
    unimplemented!(
        "Hardware channel order/sign/unit verification deferred — owned by a future bench-rig phase. \
         Production manifests must declare every channel; this hardware slot proves the manifest \
...
#[tokio::test]
#[ignore = "hardware bench — set ROZ_MANIPULATOR_HARDWARE=1 to run against real manipulator hardware"]
async fn hardware_physical_estop_latency() {
    if std::env::var("ROZ_MANIPULATOR_HARDWARE").as_deref() != Ok("1") {
        eprintln!("ROZ_MANIPULATOR_HARDWARE not set — skipping hardware physical e-stop latency test");
        return;
    }
    unimplemented!(
        "Hardware physical e-stop latency deferred — owned by a future bench-rig phase. \
