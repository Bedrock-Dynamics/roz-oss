//! FW-07 (Phase 26.10 Plan 08) — Deterministic fake manipulator backend.
//!
//! Mirrors `roz_mavlink::backend::MavlinkBackend`'s shared-state pattern: the
//! actuator and sensor halves share `Arc<Mutex<FakeManipulatorState>>` so that
...
//!   modeled — see threat T-26.10-08-02 in PLAN. Simulator-backed verticals
//!   and optional hardware bench tests cover what the fake misses.
...
impl FakeManipulatorActuator {
    /// Snapshot current joint positions for integration tests that need to
    /// prove commands sent through Copper produced simulated motion.
    #[must_use]
    pub fn joint_positions_snapshot(&self) -> Vec<f64> {
        self.0
            .lock()
            .expect("fake-manipulator mutex poisoned")
            .joint_positions
            .clone()
    }
}
...
pub fn fake_manipulator_pair(runtime: &EmbodimentRuntime) -> (FakeManipulatorActuator, FakeManipulatorSensor) {
    ...
    (
        FakeManipulatorActuator(Arc::clone(&shared)),
        FakeManipulatorSensor(shared),
    )
}
