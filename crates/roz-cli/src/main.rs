...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Some(cmd) => match cmd {
            ...
            cli::Commands::Schedule(args) => commands::schedule::execute(&args.command, &config).await,
            cli::Commands::Session(args) => commands::session::execute(&args.command, &config).await,
            ...
            cli::Commands::Mcap(args) => commands::mcap::execute(&args.command, &config).await,
