//! Reference manipulator harness vertical test.
//!
//! This validates Roz's manipulator-class task/tool/Copper path with a
//! deterministic fake backend. Simulator-backed tests own robotics realism.
...
fn manipulator_runtime(family_id: &str, manifest: &ControlInterfaceManifest) -> EmbodimentRuntime {
...
fn invocation_for(family_id: &str) -> (Uuid, TaskInvocation) {
    ...
    let runtime = manipulator_runtime(family_id, &manifest);
    let task_id = Uuid::new_v4();
    let mut invocation = TaskInvocation::new(
        task_id,
        Uuid::new_v4().to_string(),
        "Move the reference manipulator through the worker task path".into(),
        Uuid::new_v4(),
        None,
        Uuid::new_v4(),
        30,
        ...
        vec![
            CompletionResponse {
                parts: vec![ContentPart::ToolUse {
                    id: "call_promote_manipulator".into(),
            ...
            CompletionResponse {
                parts: vec![ContentPart::Text {
                    text: "Manipulator controller registered.".into(),
                }],
...
) -> roz_worker::physical_runtime::FakeManipulatorObservedState {
    let observation = result
        .manipulator_observation
        .clone()
        .expect("reference manipulator run must expose fake backend observation");
...
#[tokio::test]
async fn manipulator_task_invocation_agent_to_worker_to_fake_backend_records_state_telemetry_and_session_evidence() {
    let (task_id, mut result) = run_manipulator_case(
        "manipulator",
        &[0.6, -0.35],
...
#[tokio::test]
async fn missing_manipulator_backend_returns_structured_failure_from_task_invocation() {
    let (task_id, result) = run_manipulator_case(
        "unknown-manipulator-family",
        &[0.6, -0.35],
        WorkerTaskHarnessOptions::default(),
    )
    .await;
...
#[tokio::test]
async fn restrictive_safety_prevents_unsafe_manipulator_output_from_task_invocation() {
    let cap = 0.2;
    let (permissive_task_id, mut permissive) = run_manipulator_case(
        "manipulator",
        &[0.8, -0.75],
        WorkerTaskHarnessOptions {
            copper_policy: policy(10.0),
    ...
    let (restricted_task_id, mut restricted) = run_manipulator_case(
        "manipulator",
        &[0.8, -0.75],
        WorkerTaskHarnessOptions {
