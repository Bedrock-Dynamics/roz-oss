...
//! `crates/roz-local/src/tools/promote_controller.rs`. Drift is a bug — both
//! must register the same `ControllerCommand::load_artifact_with_embodiment_runtime`
//! shape and the same canonical name `"promote_controller"`.
//!
...
fn verify_wasm(
    ...
    for tick in 0..VERIFY_TICK_COUNT {
        ...
        let input = TickInput {
            tick,
            monotonic_time_ns: tick * 10_000_000,
            digests: DigestSet {
...
mod tests {
    ...
    fn ctx_with_runtime(
        control_manifest: roz_core::embodiment::binding::ControlInterfaceManifest,
        synthetic: bool,
    ) -> (ToolContext, mpsc::Receiver<ControllerCommand>) {
        let (tx, rx) = mpsc::channel(16);
        let mut ext = Extensions::new();
        ext.insert(tx);
        let mut runtime = test_embodiment_runtime(&control_manifest);
        if synthetic {
            runtime
                .validation_issues
    ...
    #[test]
    fn promote_controller_canonical_name() {
        let tool = PromoteControllerTool::new(&test_control_manifest());
        assert_eq!(TypedToolExecutor::name(&tool), "promote_controller");
    }

    #[tokio::test]
    async fn promote_controller_rejects_synthetic_runtime() {
        let cm = test_control_manifest();
        let tool = PromoteControllerTool::new(&cm);
        let (ctx, _rx) = ctx_with_runtime(cm, true);
        ...
        assert!(result.is_error(), "synthetic runtime must be rejected");
        assert!(
            result
                .error
                .as_deref()
    ...
    #[tokio::test]
    async fn promote_controller_promotes_valid_wasm() {
        let cm = test_control_manifest();
        let tool = PromoteControllerTool::new(&cm);
        let (ctx, mut rx) = ctx_with_runtime(cm, false);
        let input = PromoteControllerInput {
            code: valid_controller_wat(),
        };
        let result = TypedToolExecutor::execute(&tool, input, &ctx).await.unwrap();
