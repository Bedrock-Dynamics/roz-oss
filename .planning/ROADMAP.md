...
Plans:
- [ ] 26.10-01-PLAN.md — Wave 1: FW-01 — additive `embodiment_runtime: Option<EmbodimentRuntime>` on `TaskInvocation` + server-side authoritative resolver in `task_dispatch.rs:225` reading `roz_hosts.embodiment_runtime` JSONB (migration 022_host_embodiment.sql already exists; no new schema) + Phase 23 signing path unchanged + 3 unit tests + 1 server integration test
- [ ] 26.10-02-PLAN.md — Wave 1: FW-06a — manifest fidelity: replace `f64::INFINITY` synthesis at `manifest.rs:528-580` with explicit `JointSafetyLimits` parsing; preserve TCPs/grippers/sensors/workspaces; gate `SYNTHETIC_EMBODIMENT_RUNTIME_ISSUE` `#[cfg(test)]`; extend `examples/ur5/robot.toml` with explicit limits
- [ ] 26.10-03-PLAN.md — Wave 2 (depends 01+02): FW-02 — new `crates/roz-copper/src/io_factory.rs` (trait + dispatch table + `MavlinkFactory` + manipulator stub for Plan 08 to fill); new `CopperHandle::spawn_with_policy_and_io`; worker boot path at `main.rs:692` builds IO via factory and fails fast on missing runtime
