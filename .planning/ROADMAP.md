...
- [x] 26.10-02-PLAN.md — Wave 1: FW-06a — manifest fidelity: replace `f64::INFINITY` synthesis at `manifest.rs:528-580` with explicit `JointSafetyLimits` parsing; preserve TCPs/grippers/sensors/workspaces; gate `SYNTHETIC_EMBODIMENT_RUNTIME_ISSUE` `#[cfg(test)]`; extend `examples/ur5/robot.toml` with explicit limits
- [x] 26.10-03-PLAN.md — Wave 2 (depends 01+02): FW-02 — new `crates/roz-copper/src/io_factory.rs` (trait surface only; concrete `factory_for` + `MavlinkFactory`/`ManipulatorStubFactory` in `crates/roz-worker/src/io_backends.rs` per Codex H2 cycle prevention); new `CopperHandle::spawn_with_policy_and_io`; worker boot path at `main.rs:692` builds IO via factory and fails fast on missing runtime
- [ ] 26.10-04-PLAN.md — Wave 3 (depends 02+03): FW-03 — worker tool mirrors (`promote_controller`, `stop_controller`, `controller_status`) copied from `roz-local/src/tools/`; registered with `ToolCategory::Physical` at `worker/main.rs:672` for OodaReAct; `roz-local/runtime.rs:1025` extended to register live + replay side-by-side
- [ ] 26.10-05-PLAN.md — Wave 2: FW-04 — `resolve_placement` adds `has_physical_tools: bool` parameter; AUTO defaults edge when `has_host && has_physical_tools`; call site at `grpc/agent.rs:2583` consumes the already-extracted `tool_categories` HashMap (PATTERNS.md A9/Q6: zero proto changes needed)
- [ ] 26.10-06-PLAN.md — Wave 2: FW-05a+b — bounded WASM host output (`MAX_TICK_OUTPUT_BYTES = 64 KiB` guard before `vec!` at `wit_host.rs:343-361` mirroring `get_input` shape) + timer-driven stale-heartbeat scan (`tokio::time::interval(1s)` task alongside existing `safety/main.rs:38-51` watchdog; `Arc<Mutex<HeartbeatTracker>>` shared ownership)
- [ ] 26.10-07-PLAN.md — Wave 3 (depends 06): FW-05c — latched e-stop state machine (`Run → Latched → AwaitingAck → ZeroVerified → Run`) per IEC 60204-1 Stop Cat 0 + ISO 13849-1 manual reset; new `crates/roz-copper/src/latch.rs`; replace `all_default` short-circuit at `controller.rs:1111` with explicit zero emission; signed-NATS gating on `safety.estop_ack.*` + `safety.resume.*`; persist `LatchState` in WAL for fail-safe restart (Open Q5 resolved)
- [ ] 26.10-08-PLAN.md — Wave 3 (depends 03+04): FW-07a — deterministic fake-OpenClaw backend at `crates/roz-test/src/fake_openclaw.rs` (triple-trait shape mirroring `MavlinkBackend`'s shared-state `Arc<Mutex<>>`); `FakeOpenclawFactory` replaces Plan 03's stub under `test-fixtures` feature; production builds reject manipulator family with named error
- [ ] 26.10-09-PLAN.md — Wave 4 (depends 03+04+07+08): FW-06b + FW-07b — `RobotCapabilities` projection at `worker/main.rs:1394` rebuilt from `EmbodimentRuntime.model.{joints,tcps,sensor_mounts,workspace_zones}` (Pitfall 6 closure); manipulator deterministic row in `scripts/run_live_e2e_matrix.sh::run_deterministic`; gated HIL test slots (`#[ignore]` + `ROZ_OPENCLAW_HIL=1` env) for tiny bounded motion + encoder/current feedback + channel order/sign/units + physical e-stop latency; new `examples/openclaw/robot.toml` fixture
...
- [x] 26.9-02-PLAN.md — CLI surface (`roz mcap to-rrd`) + module directory skeleton with submodule stubs + feature-gated friendly error (D-17)
- [x] 26.9-03-PLAN.md — MCAP reader + `ChannelKind` classifier + warn-once state + `export_one`/`export_bulk` dispatchers (D-04/D-05/D-13/D-14/D-19/D-20)
- [x] 26.9-04-PLAN.md — `RecordingStream` builder + dual-timeline helper with B-2 method-name correction (`set_timestamp_secs_since_epoch`)
