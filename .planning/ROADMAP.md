...
- [x] 26.10-04-PLAN.md — Wave 3 (depends 02+03): FW-03 — worker tool mirrors (`promote_controller`, `stop_controller`, `controller_status`) copied from `roz-local/src/tools/`; registered with `ToolCategory::Physical` at `worker/main.rs:672` for OodaReAct; `roz-local/runtime.rs:1025` extended to register live + replay side-by-side
