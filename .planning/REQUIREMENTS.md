...
Close the production-wiring gaps surfaced by the codex review (2026-04-25) so live WASM controller deployment works through the normal agent/task path on Roz — paralleling Phase 27's drone-class validation with manipulator-class fidelity. Roz is inspired by **OpenClaw, NemoClaw, and Hermes** as agentic-harness reference architectures; these requirements validate Roz's framework claim that it can support manipulator-class hardware, not a separate vendor integration. See `26.10-CODEX-REVIEW.md` and `26.10-RESEARCH.md`.
...
- [x] **FW-07
**: Framework-fidelity fake manipulator and optional hardware-bench coverage. Deterministic in-process fake manipulator-class backend (`crates/roz-copper/src/fake_manipulator.rs`, gated by `[features] test-fixtures = []` to avoid a `roz-copper ↔ roz-test` Cargo cycle) implements the same shared-state IO pattern as `MavlinkBackend` over `Arc<Mutex<_>>`, simulating joint position/velocity only. `scripts/run_live_e2e_matrix.sh` adds a manipulator row that runs the fake-backend tests deterministically. Optional real-hardware bench tests cover tiny bounded motion, encoder/current feedback, channel order/sign/unit checks, and physical e-stop latency, gated behind `#[ignore]` + `ROZ_MANIPULATOR_HARDWARE=1`; these do not run in default CI and do not fall back to fake hardware behavior.
